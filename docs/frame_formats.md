...
| `a0`, `estia_a0` | Estia R32 A0 protocol | R32 Estia heat pumps | `EVEN` | Yes | Uses two-byte CRC-16. |
| `u`, `tu2c`, `wrapped` | TU2C wrapped protocol | Newer U-series HVAC systems | `NONE` | No, set explicitly | Work in progress. Frames are prefixed with `F0:F0` and trailing `A0`. |
| `estia` | First-generation Estia / R410A-style support | R410A Estia heat pumps | `NONE` | No, set explicitly | Uses the TU2C reader path and Estia-specific control/decoding logic. |
